...
let lastFilterSilent = false;

const PLACEHOLDER_THUMB = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='200' height='200'><rect fill='%23f0f0f0' width='200' height='200'/></svg>";

function getThumbCacheKey(sub) {
    if (sub?.processedAt) {
        return new Date(sub.processedAt).getTime();
    }
    return sub?._id || '';
}

function getPhotoThumbUrl(sub) {
    if (!sub) return PLACEHOLDER_THUMB;
    const cacheKey = getThumbCacheKey(sub);
    if (sub.photoThumb) {
        const sep = sub.photoThumb.includes('?') ? '&' : '?';
        return `${sub.photoThumb}${sep}v=${cacheKey}`;
    }
    if (!sub._id || !token) return PLACEHOLDER_THUMB;
    return `${API_BASE_URL}/submissions/${sub._id}/photo/thumb?token=${encodeURIComponent(token)}&v=${cacheKey}`;
}

function getStickerThumbUrl(sub, index) {
    if (!sub?._id || !token) return PLACEHOLDER_THUMB;
    const img = sub.generatedImages?.[index];
    const cacheKey = img?.url
        ? img.url.split('/').pop()
        : getThumbCacheKey(sub);
    return `${API_BASE_URL}/submissions/${sub._id}/sticker-thumb/${index}?token=${encodeURIComponent(token)}&v=${encodeURIComponent(cacheKey)}`;
}
...
function getActiveStatusFilter() {
    return document.querySelector('input[name="statusFilter"]:checked')?.value || 'all';
}

function onStatusFilterChange() {
    allSubmissions = [];
    paginationState.loaded = 0;
...
async function loadSubmissions(silent = false, append = false, skipAutoApprove = false) {
    ...
    try {
        ...
        let fetchLimit;
        if (append) {
            fetchLimit = paginationState.limit;
        } else if (!search && statusFilter === 'all') {
            fetchLimit = allSubmissions.length > paginationState.limit
                ? allSubmissions.length
                : paginationState.limit;
        } else {
            fetchLimit = Math.max(allSubmissions.length, paginationState.limit);
        }
        ...
        if (append) {
            ...
            const uniqueNew = newSubmissions.filter(s => !existingIds.has(s._id));
            allSubmissions = [...allSubmissions, ...uniqueNew];
        } else {
...
function filterSubmissions() {
    ...
    const activeFilter = document.querySelector('input[name="statusFilter"]:checked')?.value || 'all';
    // Completed tab must re-render so processedAt sort and sticker previews stay correct after regeneration
    const needsFullRender = activeFilter === 'completed'
        || !lastFilterSilent
        || !canPatchSubmissionsInPlace(filtered);

    if (needsFullRender) {
        displaySubmissions(filtered);
    } else {
        patchSubmissionCardsInPlace(filtered);
    }
    updateLoadMoreButton();
}

function canPatchSubmissionsInPlace(filtered) {
    const container = document.getElementById('submissionsList');
    if (!container) return false;
    const cards = container.querySelectorAll('.submission-card[data-submission-id]');
    if (cards.length !== filtered.length || cards.length === 0) return false;
    for (let i = 0; i < filtered.length; i++) {
        if (cards[i].dataset.submissionId !== String(filtered[i]._id)) return false;
    }
    return true;
}

function patchSubmissionCardsInPlace(filtered) {
    const cards = document.querySelectorAll('.submission-card[data-submission-id]');
    cards.forEach((card, i) => {
        const sub = filtered[i];
        const statusEl = card.querySelector('.submission-status');
        if (statusEl) {
            statusEl.className = `submission-status ${sub.status}`;
...
function displaySubmissions(submissions) {
    ...
    container.innerHTML = submissions.map(sub => {
        ...
        if (sub.status === 'completed' && (!sub.generatedImages || sub.generatedImages.length === 0)) {
            return `
                <motion.div class="submission-card completed-card" data-submission-id="${sub._id}">
                    <div class="submission-info" style="flex:1">
                        ...
                        <p class="failure-reason"><strong>Note:</strong> No sticker images were saved. Try Generate again or check the processor.</p>
                        <span class="submission-status ${sub.status}">${sub.status.toUpperCase()}</span>
                    </div>
                    <div class="submission-actions">
                        <button onclick="loadSubmissionForGeneration('${sub._id}')" class="btn-primary btn-sm">Generate</button>
                        <button onclick="deleteSubmission('${sub._id}')" class="btn-danger btn-sm">Delete</button>
                    </div>
                </div>
            `;
        } else if (sub.status === 'completed' && sub.generatedImages && sub.generatedImages.length > 0) {
            return `
                <div class="submission-card completed-card" data-submission-id="${sub._id}">
                    <div class="completed-preview">
                        <img class="submission-thumbnail-small" 
                             src="${getPhotoThumbUrl(sub)}"
                             alt="${sub.name}"
                             loading="lazy"
                             decoding="async">
                        <div class="sticker-thumbnails">
                            ${sub.generatedImages.slice(0, 4).map((img, idx) => `
                                <img src="${getStickerThumbUrl(sub, idx)}" 
                                     alt="Sticker ${idx + 1}" 
                                     class="sticker-thumb"
                                     loading="lazy"
                                     decoding="async"
                                     onclick='openLightbox(${JSON.stringify(sub.generatedImages)}, ${idx}, ${JSON.stringify(sub.name)}, ${JSON.stringify(String(sub.eventId?._id || sub.eventId || currentEvent._id))})'>
                            `).join('')}
                        </div>
                    </div>
            ...
            return `
                <div class="submission-card failed-card" data-submission-id="${sub._id}">
...
async function syncSubmissionAfterGeneration(submission) {
    const id = String(submission._id);
    const idx = allSubmissions.findIndex((s) => String(s._id) === id);
    if (idx >= 0) {
        allSubmissions[idx] = submission;
    } else {
        allSubmissions.unshift(submission);
    }
    if (currentSubmission && String(currentSubmission._id) === id) {
        currentSubmission = submission;
    }
    lastFilterSilent = false;
...
async function generateStickers() {
    ...
    try {
        ...
        if (currentSubmission && currentSubmission._id) {
            ...
            const queueResponse = await fetch(`${API_BASE_URL}/submissions/${submissionId}/queue-for-generation`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (!queueResponse.ok) {
                const err = await queueResponse.json().catch(() => ({}));
                throw new Error(err.error || 'Failed to queue submission for generation');
            }

            currentSubmission = await queueResponse.json();
        } else {
            const createResponse = await fetch(`${API_BASE_URL}/submissions/admin-upload`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            submissionId = newSubmission.submissionId || newSubmission.submission?._id;
        }
    ...
    generationPollInterval = setInterval(async () => {
        ...
        try {
            ...
            if (submission.status === 'completed') {
                ...
                await syncSubmissionAfterGeneration(submission);
                displayGeneratedImages(submission);
            } else if (submission.status === 'rejected' || submission.status === 'failed' || pollCount >= maxPolls) {
                clearInterval(generationPollInterval);
                generationPollInterval = null;
                await syncSubmissionAfterGeneration(submission);

                document.getElementById('generationProgress').style.display = 'none';
                const failMsg = submission.status === 'rejected'
                    ? '❌ Generation was rejected'
                    : submission.status === 'failed'
                        ? `❌ Generation failed${submission.failureReason ? ': ' + submission.failureReason : ''}`
                        : '❌ Generation timeout - check processor';
                showStatus(failMsg, 'error', 'generateTab');
...
async function displayGeneratedImages(submission) {
    ...
    showStatus('✅ Generation complete! Images are ready for printing at 300 DPI, 4"×3".', 'success', 'generateTab');
}
...
async function loadGenerationSettings() {
    ...
    generationSettings = currentEvent.generationSettings || {
        showNameOnDownload: true,
        nameLabelPosition: 'bottomLeft',
        downloadFilenameMode: 'attendeeName',
        customDownloadFilename: ''
    };
    displayGenerationSettings();
}

function displayGenerationSettings() {
    if (!generationSettings) return;

    const showNameCheckbox = document.getElementById('showNameOnDownload');
    if (showNameCheckbox) {
        showNameCheckbox.checked = generationSettings.showNameOnDownload !== false;
    }

    const modeRadio = document.querySelector(
        `input[name="downloadFilenameMode"][value="${generationSettings.downloadFilenameMode || 'attendeeName'}"]`
    );
    if (modeRadio) {
        modeRadio.checked = true;
    }

    const customFilenameInput = document.getElementById('customDownloadFilename');
    if (customFilenameInput) {
        customFilenameInput.value = generationSettings.customDownloadFilename || '';
    }

    const positionRadio = document.querySelector(
        `input[name="nameLabelPosition"][value="${generationSettings.nameLabelPosition || 'bottomLeft'}"]`
    );
    if (positionRadio) {
...
function updateGenerationSettingsView() {
    const showNameOnDownload = document.getElementById('showNameOnDownload')?.checked !== false;
    const positionSection = document.getElementById('nameLabelPositionSettings');
    if (positionSection) {
        positionSection.style.display = showNameOnDownload ? 'block' : 'none';
    }

    const mode = document.querySelector('input[name="downloadFilenameMode"]:checked')?.value || 'attendeeName';
    const customSection = document.getElementById('customDownloadFilenameSettings');
    if (customSection) {
        customSection.style.display = mode === 'customName' ? 'block' : 'none';
    }
}

async function saveGenerationSettings() {
    const statusDiv = document.getElementById('generationSettingsStatus');
    ...
    try {
        const showNameOnDownload = document.getElementById('showNameOnDownload').checked;
        const nameLabelPosition = document.querySelector('input[name="nameLabelPosition"]:checked')?.value || 'bottomLeft';
        const downloadFilenameMode = document.querySelector('input[name="downloadFilenameMode"]:checked').value;
        const customDownloadFilename = document.getElementById('customDownloadFilename').value.trim();

        if (downloadFilenameMode === 'customName' && !customDownloadFilename) {
            statusDiv.textContent = '⚠️ Custom filename prefix is required';
            statusDiv.className = 'status-message error';
            statusDiv.style.display = 'block';
            return;
        }
        ...
        if (response.ok) {
            const updatedEvent = await response.json();
            currentEvent = updatedEvent;
            generationSettings = updatedEvent.generationSettings;
            statusDiv.textContent = '✓ Generation settings saved for this event!';
            statusDiv.className = 'status-message success';
            statusDiv.style.display = 'block';
